...
public class GUI extends Application {
	...
	public void start(Stage stage) throws Exception {	//Stage pour le jeu entier
		...
		Label wDLC = new Label("Attention: ceci peut prendre du temps, et les nouvelles images DLC peuvent etre de moins bonne qualite."
				+ " Vous serez avertis lorsque ce sera pret");
		...
		/*
		 * On prend les DLC: voir controlleur
		 */
		...
		submit.setOnAction((event) -> {
			...
			if (rKitten.getText() != null && !rKitten.getText().isEmpty() &&
	        	 ...
	        	 */
	        	String strDLC = "";
	        	if(controller.hasDLC()) strDLC = "dlc/";
	    		...
	    		for(int y=0; y<firstGrid.size(); y++) {
	    			for(int x=0; x<firstGrid.get(0).size(); x++) {
	    				Image tempImg = new Image("File:nki/" + strDLC + firstGrid.get(y).get(x));
		...
		scene.setOnKeyPressed((event) -> {
			 ...
			 */
				...
				//Sinon, on traite un tour normal
	    			...
	    			Image tempRobImg = new Image("File:nki/" + rob);	//image du robot
	    			...
	    			Image tempFormerImg = new Image("File:nki/" + strDLC + former);	//image autre
